  ...
  const {
   ...
   const renderScores = (frame, roll) => {
    ...
    if (frames[frame]) {
      ...
      if (frames[frame][roll] === 0)
      return '-'
      else if (frames[frame][roll] === 10)
      return 'X'
      else if (frames[frame][roll] + roll === 10)
      return'/'
      else
    ...
    return (
      <div className='Container'>
        ...
        {rolls > 0 &&
          <button className={'Reset'} onClick={() =>dispatch({ type: types.reset })}>Reset</button>
        }
      </div>
    )
}
